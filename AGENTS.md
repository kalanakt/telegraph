...
**Billing enforcement**: Clerk handles auth, and Creem handles billing. Webhook at `/api/webhooks/creem` syncs subscription status to the DB. Orchestrator checks entitlements via `packages/shared/src/config/limits.ts` before processing.
...
Two Railway services: `web` (Next.js) and `worker` (Node.js). Use the `railway:*` scripts in the root `package.json`.
