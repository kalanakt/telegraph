...
export const botStatusEnum = pgEnum('bot_status', ['active', 'paused', 'deleted']);
// tenants table
...
export const flows = pgTable('flows', {
    ...
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [index('flows_bot_id_idx').on(t.botId), index('flows_tenant_id_idx').on(t.tenantId)]);
// published_plans table
export const publishedPlans = pgTable('published_plans', {
    id: uuid('id').primaryKey().defaultRandom(),
    flowId: uuid('flow_id')
        .notNull()
        .references(() => flows.id),
...
}, (t) => [uniqueIndex('raw_updates_bot_update_idx').on(t.botId, t.telegramUpdateId)]);
