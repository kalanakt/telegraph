...
import { verifyWebhookSecret, telegramApiUrl } from './telegram';

describe('verifyWebhookSecret', () => {
  it('returns true for matching secrets', () => {
    expect(verifyWebhookSecret('my-secret-value', 'my-secret-value')).toBe(
      true,
    );
  });
...
describe('telegramApiUrl', () => {
  it('returns correct URL', () => {
    expect(telegramApiUrl('123:ABC', 'sendMessage')).toBe(
