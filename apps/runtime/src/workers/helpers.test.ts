...
describe("renderTemplate", () => {
  ...
  it("JSON-stringifies object values", () => {
    expect(renderTemplate("data: {{obj}}", { obj: { a: 1 } })).toBe(
      'data: {"a":1}',
    );
  });
  ...
  it("returns template unchanged when there are no placeholders", () => {
    expect(renderTemplate("no placeholders here", { x: "1" })).toBe(
...
describe("evaluateCondition", () => {
  ...
  it("resolves default edge by edge id when compiler passes edge id", () => {
    const conditionEdges = [
      { condition: "rule-yes", targetNodeId: "yes-target" },
      { condition: undefined, targetNodeId: "no-target" },
    ];
    const rules = [
      {
        variable: "answer",
    ...
    expect(result).toEqual({ condition: undefined, targetNodeId: "no-target" });
  });
  ...
  it("returns null when no rules match and no default", () => {
    ...
    const result = evaluateCondition(rules, strictEdges, { answer: "no" });
