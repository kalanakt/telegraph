    ...
    return createWorker(QUEUE_NAMES.EXECUTE, async (job) => {
        ...
        try {
            ...
            if (updateData['userInput'] !== undefined) {
                ...
                const waitVar = session.variables['_waitVariable'];
                if (waitVar) {
            ...
            while (currentNodeId && steps < MAX_STEPS) {
                steps++;
                const node = cached.plan.nodes[currentNodeId];
                if (!node)
                    break;
                ...
                switch (node.type) {
                    ...
                    case 'http_request': {
                        ...
                        try {
                            ...
                            if (cfg.headers)
                                fetchOpts.headers = cfg.headers;
                            if (cfg.body)
                                fetchOpts.body = renderTemplate(cfg.body, session.variables);
                    ...
                    case 'condition': {
                        const cfg = node.config;
                        const matchedEdge = evaluateCondition(cfg.rules, node.edges, session.variables, cfg.defaultEdgeId);
                        currentNodeId = matchedEdge?.targetNodeId ?? null;
                        break;
                    }
                    case 'set_variable': {
                        const cfg = node.config;
                        session.variables[cfg.variable] = evaluateExpression(cfg.valueExpression, session.variables);
                        currentNodeId = node.edges[0]?.targetNodeId ?? null;
                        break;
                    }
                    case 'wait_for_input': {
