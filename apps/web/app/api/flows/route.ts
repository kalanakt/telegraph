...
function mapFlowError(error: unknown, fallback: string) {
  ...
  if (
    ...
    return "Your database enum is behind the app schema. Run the latest Prisma migration or `pnpm prisma:push`, then try saving the flow again.";
  }
...
export async function POST(req: Request) {
  try {
    ...
    const message = mapFlowError(error, "Failed to create flow");
...
export async function PUT(req: Request) {
  try {
    ...
    const message = mapFlowError(error, "Failed to update flow");
    return NextResponse.json({ error: message }, { status: 400 });
  }
}
