...
export default async function botRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/", async (request, reply) => {
    ...
    const webhookBaseUrl =
      body.webhookBaseUrl ?? process.env["DEFAULT_WEBHOOK_BASE_URL"];
    ...
    try {
      ...
      const webhookError =
        error instanceof Error
          ? error.message
          : "Unable to auto-register webhook.";

      request.log.warn(
        { botId: bot.id, tenantId: request.user.tenantId, webhookError },
        "Auto webhook registration failed after bot creation",
      );
  ...
  fastify.post<{ Params: { botId: string } }>(
    "/:botId/test-message",
    async (request, reply) => {
      const body = parseBody(SendTestMessageBody, request.body);
