...
import { createBot, deleteBot, getBot, listBots, registerWebhook, removeWebhook, updateBot, } from '../services/bot.service.js';
...
export default async function botRoutes(fastify) {
    ...
    fastify.get('/', async (request, reply) => {
        ...
        const result = await listBots(fastify.db, request.user.tenantId, Math.min(parseInt(limit), 100), parseInt(offset));
    ...
    fastify.get('/:botId', async (request, reply) => {
    ...
    fastify.patch('/:botId', async (request, reply) => {
    ...
    fastify.delete('/:botId', async (request, reply) => {
        await deleteBot(fastify.db, request.user.tenantId, request.params.botId);
        return reply.code(204).send();
    });
    fastify.post('/:botId/webhook/register', async (request, reply) => {
        const body = parseBody(RegisterWebhookBody, request.body);
        const result = await registerWebhook(fastify.db, request.user.tenantId, request.params.botId, getMasterKey(), body.webhookBaseUrl);
        return reply.send(result);
    });
    fastify.post('/:botId/webhook/remove', async (request, reply) => {
